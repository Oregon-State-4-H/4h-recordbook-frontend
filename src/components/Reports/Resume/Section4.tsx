...
function addPageBreaks() {
  ...
  while (i < rows.length) {
    rows.splice(
      i,
      0,
      <TableHeader headerKey={"Sec4Head-" + count} isBreak={true} />
    );
    i += 25;
    count++;
  }
 ...
 */
export default function Section4Report(props: ResumePDFProps) {
  const tableData: Section4[] = props.tableData.filter((item) => isS4(item));
  ...
  return (
    <Page size="LETTER" style={ReportStyles.body} wrap>
      ...
      <Text style={ReportStyles.tableHeaading}>
        List of all non-4-H activities and event I have attended.
      </Text>
