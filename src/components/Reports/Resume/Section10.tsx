...
function addPageBreaks() {
  ...
  while (i < rows.length) {
    rows.splice(
      i,
      0,
      <TableHeader headerKey={"Sec10Head-" + count} isBreak={true} />
    );
    i += 25;
    count++;
  }
 ...
 */
export default function Section10Report(props: ResumePDFProps) {
  const tableData: Section10[] = props.tableData.filter((item) => isS10(item));
    ...
    return (
      <View key={index} style={styles.tableRow}>
        ...
        <View style={[styles.col2, ReportStyles.tableColAlignLeft]}>
          <Text>{row.communication_type}</Text>
        </View>
        <View style={[styles.col3, ReportStyles.tableColAlignLeft]}>
          <Text>{row.topic}</Text>
        </View>
        <View style={[styles.col4, ReportStyles.tableColAlignCenter]}>
          <Text>{row.times_given}</Text>
        </View>
  ...
  return (
    <Page size="LETTER" style={ReportStyles.body} wrap>
      ...
      <Text style={ReportStyles.tableHeaading}>
        Communication not related to 4-H
      </Text>
