...
function addPageBreaks() {
  ...
  while (i < rows.length) {
    rows.splice(
      i,
      0,
      <TableHeader headerKey={"Sec5Head-" + count} isBreak={true} />
    );
    i += 25;
    count++;
  }
 ...
 */
export default function Section5Report(props: ResumePDFProps) {
  const tableData: Section5[] = props.tableData.filter((item) => isS5(item));
  ...
  return (
    <Page size="LETTER" style={ReportStyles.body} wrap>
      ...
      <Text style={ReportStyles.tableHeaading}>
        List of all 4-H related leadership roles
      </Text>
