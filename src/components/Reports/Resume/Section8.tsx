...
function addPageBreaks() {
  ...
  while (i < rows.length) {
    rows.splice(
      i,
      0,
      <TableHeader headerKey={"Sec8Head-" + count} isBreak={true} />
    );
    i += 25;
    count++;
  }
...
/**
 * PDF page for Section 8 of the 4-H Resume
 * @param {Object} tableData - resume data for section 8
 * @returns {JSX.Element}
 * @see {@link 'src/app/_db/models/resumeSections/section8Model'} for object structure
 * @example <Section8 tableData={section8Data}/>
 */
export default function Section8Report(props: ResumePDFProps) {
  const tableData: Section8[] = props.tableData.filter((item) => isS8(item));
    ...
    return (
      <View key={index} style={styles.tableRow}>
        ...
        <View style={[styles.col2, ReportStyles.tableColAlignLeft]}>
          <Text>{row.individual_group_activities}</Text>
        </View>
        <View style={[styles.col3, ReportStyles.tableColAlignCenter]}>
  ...
  return (
    <Page size="LETTER" style={ReportStyles.body} wrap>
      ...
      <Text style={ReportStyles.tableHeaading}>
        List of all non-4-H related activitied that contributed to the welfare
        of my club or community
      </Text>
