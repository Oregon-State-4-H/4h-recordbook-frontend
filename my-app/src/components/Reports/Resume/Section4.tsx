...
import { Section4 } from "@/API/ResumeAPI";
    ...
    return (
      <View key={headerKey} style={styles.headerRow}>
        ...
        <View style={[styles.col3, ReportStyles.tableColAlignCenter]}>
          <Text>Things I Did and Time Spent</Text>
        </View>
...
function addPageBreaks() {
  ...
  while (i < rows.length) {
    rows.splice(i, 0, <TableHeader key={"Sec4Head-" + count} isBreak={true} />);
    i += 25;
    count++;
  }
}
...
/**
 * PDF page for Section 4 of the 4-H Resume
 * @param {Object} tableData - resume data for section 4
 * @returns {JSX.Element}
 * @see {@link 'src/app/_db/models/resumeSections/section4Model'} for object structure
 * @example <Section4 tableData={section4Data}/>
 */
export default function Section4Report(props: any) {
  const tableData: Section4[] = props.tableData;
  ...
  return (
    <Page size="LETTER" style={ReportStyles.body} wrap>
        <Text style={ReportStyles.h1}>Section 4: Participation in Other Community Activities/Events</Text>
        <Text style={ReportStyles.tableHeaading}>List of all non-4-H activities and event I have attended.</Text>
  
        <TableHeader headerKey={"Sec4Head-0"} isBreak={false} />
